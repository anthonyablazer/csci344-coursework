...
/**
 * Generate Post
 * 1) Go out to the internet and fetch all posts
 * 2) Loop through the recieved posts and append each post to the correct place
 */
async function getPosts() {
    const response = await fetch("https://photo-app-secured.herokuapp.com/api/posts/?limit=20", {
        method: "GET",
        headers: {
    ...
    //Draw posts to screen with showPosts()
    showPosts(data);
}
...
function showPosts(posts){
    ...
    //Loop through posts and insert HTML representation into main
    posts.forEach(post => {
        const template = 
        `<section class="bg-white border mb-10">
            <div class="p-4 flex justify-between">
                <h3 class="text-lg font-Comfortaa font-bold">${post.user.username}</h3>
                <button class="icon-button" aria-label="more button"><i class="fas fa-ellipsis-h"></i></button>
            </div>
